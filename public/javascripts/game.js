...
function createMenuGroup() { //function createInventoryMenuGroup() {
    ...
    for (i = 0; i < 40; i++) {
        ...
        gui.inventory.socket[i].background = game.make.graphics();
...
function buildInventoryMenu() { //function buildInventoryMenu() {
    ...
    for (var i = 0; i < 40; i++) {
        if (player.inventory[i].tileIndex != -1) {
            var sprite = game.make.sprite(4, 4, 'tilesSprite', player.inventory[i].tileIndex - 1);
            var text = '';
            text += player.inventory[i].tileQuantity;
            var textSprite;
            if (player.inventory[i].tileQuantity < 10) {
                textSprite = game.add.text(22, 16, text, style);
            } else {
                textSprite = game.add.text(15, 16, text, style);
            }
